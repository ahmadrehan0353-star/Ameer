...
function wireStep() {
  ...
  if (step === 4) {
    ...
    $("placeBtn").onclick = async () => {
      ...
      try {
        // Last line of defense against overselling AND price tampering:
        // re-read each item fresh from Firestore (not the cached catalog)
        // right before we commit. Stock may have sold out to someone else
        // since this page loaded, and price/salePrice on the client could
        // have been edited in devtools or intercepted in transit — so we
        // overwrite both from the live document before computing totals.
        // This works with zero extra infrastructure because /products is
        // already publicly readable in firestore.rules; Firestore itself
        // is the source of truth, we just weren't re-checking price before.
        const { db, doc, getDoc, updateDoc, increment } = await import("./firebase.js");
        for (const l of lines) {
          const snap = await getDoc(doc(db, "products", l.p.id));
          if (!snap.exists()) {
            toast(`"${l.p.name}" is no longer available`, "err");
            btn.disabled = false; btn.textContent = "Place order";
            return;
          }
          const data = snap.data();
          const live = data.stock ?? 0;
          if (live < l.qty) {
            toast(`Sorry — only ${live} of "${l.p.name}" left in stock`, "err");
            btn.disabled = false; btn.textContent = "Place order";
            return;
          }
          // Overwrite the cached price with the live one. l.p is the same
          // object reference stored in the module-level `products` array,
          // so this also fixes what totals()/summaryAside() show next.
          l.p.price = data.price;
          l.p.salePrice = data.salePrice ?? null;
        }
