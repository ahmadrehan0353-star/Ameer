...
async function totals() {
  ...
  return { subtotal, couponable, discount, shipping, tax, total: afterDiscount + shipping + tax, couponId, coupon: discount ? code : null };
}
...
async function summaryAside() {
  ...
  return `<aside class="summary">
    <h3>Your order</h3>
    ${lines.map((l) => `<div class="co-review-item"><span>${esc(l.p.name)} <span class="muted">×${l.qty}</span>${l.p.noCoupon ? `<span style="display:block;color:#b0453a;font-size:.72rem;font-weight:600">Discount isn't available on this item</span>` : ""}</span><span>${money((l.p.salePrice||l.p.price)*l.qty)}</span></div>`).join("")}
    ${(() => {
      const excluded = lines.filter((l) => l.p.noCoupon).length;
      return t.coupon && excluded
        ? `<h4 style="margin:10px 0 2px;color:#b0453a;font-size:.85rem">Discount not applied to ${excluded} item${excluded > 1 ? "s" : ""}</h4>
           <p class="muted" style="font-size:.74rem;margin:0">Your code only discounts the eligible items — coupons aren't available on the item${excluded > 1 ? "s" : ""} marked above.</p>`
        : "";
    })()}
