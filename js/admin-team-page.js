...
import { listTeam, inviteTeammate, activateTeammate, resendCode, removeTeammate, emailJsConfigured, isPrimaryAdmin } from "./admin-team.js";
...
(async function () {
  ...
  // Only owner accounts (listed in ADMIN_EMAILS) may manage the team.
  // An invited teammate must not be able to add admins or remove the owner.
  if (!(await isPrimaryAdmin(me.email))) {
    body.innerHTML = `
      ...
      <div class="ad-panel">
        ...
        <p style="font-size:.9rem;color:var(--ink-soft)">
          You're signed in as <b>${me.email}</b>, a team admin. Only the store owner can invite
          or remove admins. Ask the owner if you need access changed.
        </p>
      </div>`;
    return;
  }
