...
// eligibleSubtotal (optional): the part of the subtotal made up of products
// that allow coupons (i.e. excluding items flagged noCoupon in the admin).
// When omitted, the whole subtotal is treated as eligible — so existing
// callers keep working unchanged.
export async function validateCoupon(code, subtotal = 0, eligibleSubtotal = null) {
  ...
  // Discount is worked out on the coupon-eligible amount only. If every
  // item in the bag is coupon-excluded, say so instead of silently
  // applying Rs 0 off.
  const base = eligibleSubtotal == null ? subtotal : eligibleSubtotal;
  if (base <= 0 && subtotal > 0) {
    return { ok: false, reason: "Coupons can't be used on the items in your bag" };
  }
  const discount = c.type === "percent" ? base * (c.value / 100) : Math.min(c.value, base);
