...
export async function createTeammateAccount(email, password) {
  ...
  try {
    ...
    // The Firestore rules only treat a verified email as an admin, so the
    // teammate MUST verify before the dashboard will work for them. This is
    // the one moment we're holding their session, so send it now.
    try { await sendEmailVerification(cred.user); }
    catch (e) { console.warn("teammate verification email:", e); }
