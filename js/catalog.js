...
// Type chips depend on which Category chip(s) are currently selected —
// pick "Unstitched" and its types (Master Copy, Original, …) appear; pick
// nothing and the group stays empty with a hint. Type is a multi-value
// field on the product (product.quality), so a product can match more than
// one Type chip.
function buildTypeFilters() {
  ...
  if (!state.subs.size) {
    state.types.clear();
    el.innerHTML = `<span class="muted" style="font-size:.82rem">Pick a category above to see its types</span>`;
    return;
  }

  // types that are present on a product in this dataset whose category is
  // currently selected
  const available = new Set();
  state.all.forEach((p) => {
    if (state.subs.has(p.sub)) (p.quality || []).forEach((q) => available.add(q));
  });
  ...
  if (!available.size) {
    el.innerHTML = `<span class="muted" style="font-size:.82rem">No types for this selection yet</span>`;
    return;
  }
