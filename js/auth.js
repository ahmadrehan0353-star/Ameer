...
/* ---------------- orders ---------------- */
// A short, human-typable order number: 1 letter + 4 digits (e.g. "K2847").
// Ambiguous characters (0/O, 1/I/L, 2/Z, 6/G) are left out of both sets so
// people can read it off a screen or read it back over the phone.
const CODE_LETTERS = "ACDEFGHJKMNPQRTUVWXY";
const CODE_DIGITS = "345789";
...
function makeOrderCode() {
  return randChar(CODE_LETTERS) + Array.from({ length: 4 }, () => randChar(CODE_DIGITS)).join("");
}
...
export async function placeOrder(user, order) {
  ...
  // lookup. Short codes collide more often than long ones, and a collision
  // here would silently overwrite someone else's order — so check the
  // number is actually free first, and pick a new one on the rare occasion
  // it isn't (a handful of tries is effectively always enough).
