// ============================================================
// AMEER OFFICIAL — reusable product card + wishlist/quick-add wiring
// ============================================================
import { money, esc, lsGet, lsSet, placeholder, optimizeImg } from "./utils.js";
...
export function productCard(p) {
  ...
  const rawImg = p.image || placeholder(p.id);
  const rawAlt = (p.images && p.images[1]) || rawImg;
  const img = optimizeImg(rawImg, 500);
  const alt = optimizeImg(rawAlt, 500);
  return `<article class="p-card">
    <div class="p-frame" onclick="location.href='product.html?id=${esc(p.id)}'">
      ${badges.length ? `<div class="p-badges">${badges.join("")}</div>` : ""}
      <img class="main" loading="lazy" src="${esc(img)}" data-raw="${esc(rawImg)}" alt="${esc(p.name)}" onerror="if(this.src!==this.dataset.raw){this.src=this.dataset.raw}else{this.style.opacity=0}">
      <img class="alt" loading="lazy" src="${esc(alt)}" data-raw="${esc(rawAlt)}" alt="" onerror="if(this.src!==this.dataset.raw){this.src=this.dataset.raw}else{this.style.display='none'}">
