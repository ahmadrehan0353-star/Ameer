...
import { money, esc, lsGet, lsSet, param, toast } from "./utils.js";
    ...
    <div class="pdp">
      <div class="gallery">
        <div class="gal-main" id="galMain">
          <img id="galMainImg" src="${esc(images[state.img])}" alt="${esc(product.name)}">
        </div>
        ${images.length > 1 ? `<div class="gal-thumbs" id="galThumbs">
          ${images.map((src, i) => `<button class="${i === state.img ? "on" : ""}" data-i="${i}"><img src="${esc(src)}" alt=""></button>`).join("")}
        </div>` : ""}
      </div>
      <div class="pdp-info">
        <div class="pdp-brand">AMEER OFFICIAL</div>
        ...
        <div class="pdp-perks">
          <div class="pdp-perk"><svg viewBox="0 0 24 24"><path d="M3 12l6 6L21 6"/></svg>Free shipping over Rs 5000</div>
          <div class="pdp-perk"><svg viewBox="0 0 24 24"><path d="M3 12l6 6L21 6"/></svg>Free 30-day returns</div>
        ...
        <details class="acc">
          ...
          <div class="acc-body">Orders ship within 2–4 business days. Free 30-day returns on unworn items with tags attached.</div>
        </details>
      </div>
    </div>`;
...
function wirePDP(images) {
  ...
  document.querySelectorAll("#galThumbs button").forEach((btn) => {
    btn.onclick = () => {
      ...
      document.getElementById("galMainImg").src = images[state.img];
