    ...
    <div class="pdp">
      <div class="gallery">
        <div class="gal-main" id="galMain">
          <img id="galMainImg" src="${esc(optimizeImg(images[state.img], 1000))}" alt="${esc(product.name)}">
        </div>
        ${images.length > 1 ? `<div class="gal-thumbs" id="galThumbs">
          ${images.map((src, i) => `<button class="${i === state.img ? "on" : ""}" data-i="${i}"><img src="${esc(optimizeImg(src, 150))}" alt=""></button>`).join("")}
        </div>` : ""}
      </div>
      <div class="pdp-info">
...
function wirePDP(images) {
  ...
  document.querySelectorAll("#galThumbs button").forEach((btn) => {
    btn.onclick = () => {
      ...
      document.getElementById("galMainImg").src = optimizeImg(images[state.img], 1000);
