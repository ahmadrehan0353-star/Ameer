// ============================================================
// AMEER OFFICIAL — account / profile page
// ============================================================
import { renderNavbar } from "./navbar.js";
import { renderFooter } from "./app.js";
import { onUser, getProfile, saveAddresses, getOrders, logOut, formatOrderCode } from "./auth.js";
...
async function boot() {
  ...
  onUser(async (u) => {
    if (!u) { location.href = "login.html?next=" + encodeURIComponent("profile.html" + location.search); return; }
    user = u;
...
function render() {
  ...
  if (wantTab) {
    const btn = document.querySelector(`.acct-nav [data-tab="${wantTab}"]`);
    if (btn) btn.click();
  }
...
function orderRow(o) {
  ...
  return `<div class="order-row">
    <div class="order-top">
      <div><b>${esc(formatOrderCode(o.id))}</b><br><span class="muted">${date}</span></div>
