...
async function buildCategoryStrip() {
  ...
  wrap.innerHTML = tiles.map((t) => `
    <a class="cat-tile" href="${t.link}"><img loading="lazy" src="${optimizeImg(t.image, 600)}" alt="${t.name}"><b>${t.name}<small>${t.subtitle}</small></b></a>
  `).join("");
}
...
function buildHero() {
  ...
  wrap.innerHTML = SLIDES_ACTIVE.map((s, i) => `
    <div class="hero-slide ${i===0?'on':''}">
      <img ${i === 0 ? `src="${optimizeImg(s.img, 1600)}" fetchpriority="high"` : `data-src="${optimizeImg(s.img, 1600)}"`} alt="">
