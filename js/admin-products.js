...
function openEditor(p) {
  ...
  box.innerHTML = `
    ...
    <div class="ad-form-grid">
      ...
      <div class="field"><label>Subcategory</label><select id="e-subcat"></select></div>
  ...
  fillSubcats(p?.category || "women", document.getElementById("e-sub").value, p?.subcategory);
  renderColors(); renderSizes(); renderImages();
  workQuality = Array.isArray(p?.quality) ? [...p.quality] : [];
  loadBrandAndQuality(p?.brand);

  document.getElementById("e-cat").onchange = (e) => {
    fillSubs(e.target.value);
    fillSubcats(e.target.value, document.getElementById("e-sub").value);
  };
  document.getElementById("e-sub").onchange = (e) =>
    fillSubcats(document.getElementById("e-cat").value, e.target.value);
...
// Third-level dropdown, dependent on both Category and Sub-category — e.g.
// women + Unstitched → Lawn, Cotton, Chiffon, …
function fillSubcats(cat, sub, selected) {
  const el = document.getElementById("e-subcat");
  if (!el) return;
  const options = (SUBCATEGORIES[String(cat || "women").toLowerCase()] || {})[sub] || [];
  el.innerHTML = `<option value="">— none —</option>` + options.map((s) =>
    `<option value="${esc(s)}" ${s === selected ? "selected" : ""}>${esc(s)}</option>`).join("");
}
