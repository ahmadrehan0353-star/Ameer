...
import { adminListProducts, adminSaveProduct, adminDeleteProduct, adminDuplicateProduct, uploadImage, listCategories, cloudinaryReady,
         listBrands, saveBrand, listQualities, saveQuality } from "./admin-data.js";
...
function openEditor(p) {
  ...
  box.innerHTML = `
    ...
    <div class="ad-form-grid">
      ...
      <div class="field full">
        <label>Type <span style="font-weight:400;color:var(--ink-soft);font-size:.8rem">— tap to select one or more</span></label>
        <div class="fgroup" id="qualityList" style="margin-bottom:8px"></div>
        <div style="display:flex;gap:8px;align-items:center">
          <button type="button" class="mini-btn gold" id="showAddQuality">+ Add option</button>
        </div>
...
function renderQuality() {
  ...
  el.innerHTML = list.map((q) =>
    `<button type="button" class="chip ${workQuality.includes(q.name) ? "on" : ""}" data-q="${esc(q.name)}">${esc(q.name)}</button>`).join("")
    || `<span class="muted" style="font-size:.85rem">No options yet — add one below.</span>`;
  el.querySelectorAll("[data-q]").forEach((b) => b.onclick = () => {
    const v = b.dataset.q;
    const i = workQuality.indexOf(v);
    if (i >= 0) workQuality.splice(i, 1); else workQuality.push(v);
    renderQuality();
  });
}
