...
import { adminListProducts, adminSaveProduct, adminDeleteProduct, adminDuplicateProduct, uploadImage, listCategories, cloudinaryReady,
         listBrands, saveBrand, listQualities, saveQuality, deleteQuality,
         listSubcategories, saveSubcategory, deleteSubcategory } from "./admin-data.js";
...
let brands = [], qualities = [], workQuality = [], subcategories = [];
...
function renderRows() {
  ...
  let list = products.filter((p) =>
    (!q || (p.name + p.sub + p.category + (p.subcategory || "")).toLowerCase().includes(q)) &&
    (!cat || p.category === cat) &&
  ...
  document.getElementById("rows").innerHTML = list.map((p) => {
    ...
    return `<tr>
      ...
      <td>${esc(p.category)}<br><span class="muted" style="font-size:.78rem">${esc(p.sub || "")}${p.subcategory ? " › " + esc(p.subcategory) : ""}</span></td>
...
function openEditor(p) {
    ...
    <div class="ad-form-grid">
      ...
      <div class="field full">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          <select id="e-subcat" style="flex:1"></select>
          <button type="button" class="mini-btn gold" id="showAddSubcat">+ Add subcategory</button>
        </div>
        <div id="addSubcatRow" style="display:none;gap:8px;margin-top:8px">
          <input id="newSubcat" placeholder="New subcategory name" style="flex:1;padding:9px 12px;border:1.5px solid var(--line-strong);border-radius:8px;background:var(--surface);color:var(--ink)">
          <button type="button" class="mini-btn" id="saveNewSubcat">Save</button>
          <button type="button" class="mini-btn" id="cancelNewSubcat">Cancel</button>
        </div>
      </div>
  ...
  loadSubcategories(p?.subcategory);
...
async function save() {
  ...
  const product = {
    ...
    subcategory: document.getElementById("e-subcat").value || null,
...
async function loadSubcategories(selected) {
  subcategories = await listSubcategories();
  renderSubcatSelect(selected);
}
function renderSubcatSelect(selected) {
  const el = document.getElementById("e-subcat");
  if (!el) return;
  const list = [...subcategories].sort((a, b) => (a.order || 0) - (b.order || 0));
  el.innerHTML = `<option value="">— none —</option>` + list.map((s) =>
    `<option value="${esc(s.name)}" ${s.name === selected ? "selected" : ""}>${esc(s.name)}</option>`).join("");
}
function wireSubcat() {
  const toggle = (rowId, show) => { const r = document.getElementById(rowId); if (r) r.style.display = show ? "flex" : "none"; };
  document.getElementById("showAddSubcat").onclick = () => { toggle("addSubcatRow", true); document.getElementById("newSubcat").focus(); };
  document.getElementById("cancelNewSubcat").onclick = () => toggle("addSubcatRow", false);
  document.getElementById("saveNewSubcat").onclick = async () => {
    const name = document.getElementById("newSubcat").value.trim();
    if (!name) { toast("Enter a subcategory name", "err"); return; }
    if (subcategories.some((s) => s.name.toLowerCase() === name.toLowerCase())) { toast("That subcategory already exists", "err"); return; }
    try {
      await saveSubcategory({ name, order: subcategories.length + 1 });
      subcategories = await listSubcategories();
    } catch (e) {
      console.error("saveSubcategory failed:", e);
      toast(saveErrorText(e, "subcategory"), "err");
      return;
    }
    renderSubcatSelect(name);
    document.getElementById("newSubcat").value = "";
    toggle("addSubcatRow", false);
