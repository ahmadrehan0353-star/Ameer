...
function openEditor(p) {
  ...
  box.innerHTML = `
    ...
    <div class="field full"><label>Discounts</label>
      <div style="display:flex;gap:16px;flex-wrap:wrap">
        <label><input type="checkbox" id="e-nocoupon" ${p?.noCoupon ? "checked" : ""}> Coupons can't be used on this product</label>
      </div>
    </div>
...
async function save() {
  ...
  const product = {
    ...
    // When true, this product's amount is excluded from coupon discounts
    // (see validateCoupon in coupons.js — cart.js/checkout.js pass in the
    // coupon-eligible portion of the subtotal).
    noCoupon: document.getElementById("e-nocoupon").checked,
