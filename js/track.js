...
import { trackOrder } from "./auth.js";
...
function renderNotFound(code) {
  root.innerHTML = `
    <div class="track-empty">
      <b>No order found for "${esc(code)}"</b>
      <p>Please check the number and try again. It looks like <b>AO-XXXXXXXX</b> and is on your confirmation screen.</p>
    ...
    <div class="track-card">
      <div class="track-head">
        <div>
          ...
          <span class="track-id">${esc(o.id)}</span>
        </div>
...
async function lookup(codeRaw) {
  ...
  root.innerHTML = `<div class="track-loading">Looking up ${esc(code)}…</div>`;
  try {
...
(function boot() {
  ...
  // Arriving straight from checkout: ?id=AO-XXXXXXXX
  const pre = param("id");
  if (pre) { input.value = pre.toUpperCase(); lookup(pre); }
})();
